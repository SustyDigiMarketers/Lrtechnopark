...
export const TrustedMarquee: React.FC = () => {
  ...
  return (
    <section className="py-8 bg-slate-950 border-b border-slate-900 overflow-hidden text-slate-400 select-none">
      ...
      <div className="relative w-full overflow-hidden">
        ...
        <div className="flex items-center gap-4 sm:gap-6 animate-marquee whitespace-nowrap">
          {[...signalBadges, ...signalBadges].map((item, idx) => (
            <div
              key={`${item.title}-${idx}`}
              className="inline-flex items-center gap-3 px-4 py-2 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-blue-500/40 text-slate-300 transition-colors shrink-0 group cursor-default"
            >
              <div className="w-6 h-6 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 text-xs font-mono font-bold group-hover:scale-105 transition-transform">
