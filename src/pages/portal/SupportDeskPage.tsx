      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-200">
          ...
          <button
            ...
            className="px-6 py-3.5 rounded-full bg-[#1a56db] hover:bg-[#1545b3] text-white font-bold text-xs flex items-center gap-2 transition-all cursor-pointer shadow-md shadow-blue-600/20 shrink-0 self-start md:self-auto active:scale-98"
          >
        ...
        {isCreatingTicket && (
          <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200/90 shadow-xl space-y-4">
            <div className="border-b border-slate-100 pb-3">
              <h3 className="text-lg font-bold text-slate-900">Log Technical Support Incident</h3>
              <p className="text-xs text-slate-500 mt-0.5">Response guaranteed within 15 minutes for SEV-1 incidents</p>
            </div>

            <form onSubmit={handleCreateTicket} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Incident Subject *</label>
                  <input
                    type="text"
                    required
                    value={subject}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs text-slate-900 focus:outline-none focus:border-[#1a56db] focus:bg-white font-medium"
                  >
                    <option value="SEV_3_NORMAL">SEV-3 Normal (General configuration / Question)</option>
                    <option value="SEV_2_HIGH">SEV-2 High (Degraded performance / Impending deadline)</option>
                    <option value="SEV_1_CRITICAL">SEV-1 Critical (Service Outage / Production Down)</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Related Order Reference ID</label>
                <input
                  type="text"
                  value={orderRef}
                  onChange={(e) => setOrderRef(e.target.value)}
                  placeholder="LRTP-2026-000101"
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs text-slate-900 font-mono placeholder-slate-400 focus:outline-none focus:border-[#1a56db] focus:bg-white"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Detailed Description & Diagnostics *</label>
                <textarea
                  required
                  rows={3}
                  value={description}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-5 py-2 rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 text-xs font-bold cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-6 py-2 rounded-full bg-[#1a56db] hover:bg-[#1545b3] text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-md shadow-blue-600/20 active:scale-98"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {tickets.map((t) => (
              <div
                key={t.id}
                className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-blue-200 transition-colors"
              >
                ...
                <div className="text-left sm:text-right text-xs">
                  <span className="font-mono font-semibold text-emerald-600">15-Min SLA Active</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-7 rounded-3xl bg-white border border-slate-200/90 shadow-xs">
          <div className="flex items-start gap-3.5">
            <Phone className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
            <div className="text-xs">
              <div className="font-bold text-slate-900">Direct 24/7 Redundant NOC Hotline</div>
              <div className="text-slate-600 font-mono mt-0.5">+1 (800) 578-3246 (Option 1 for SEV-1 emergencies)</div>
            </div>
          </div>

          <div className="flex items-start gap-3.5">
            <Mail className="w-5 h-5 text-[#1a56db] shrink-0 mt-0.5" />
            <div className="text-xs">
              <div className="font-bold text-slate-900">Emergency Pager Duty Escalation</div>
              <div className="text-slate-600 font-mono mt-0.5">noc-pager@lrtechnopark.com</div>
            </div>
          </div>
        </div>
      </div>
